              ...
              {question.solutionSteps && (
                <div className="mt-3 pt-3 border-t border-emerald-200 dark:border-emerald-900/60">
                  ...
                  <div className="space-y-1.5 text-xs text-gray-700 dark:text-gray-300">
                    {(() => {
                      // Group consecutive HTML lines into single blocks so tables
                      // render as complete DOM fragments instead of fragmented divs
                      const groups: Array<{ isHtml: boolean; content: string; startIdx: number }> = [];
                      let i = 0;
                      while (i < (question.solutionSteps?.length ?? 0)) {
                        const step = question.solutionSteps![i];
                        if (step.trimStart().startsWith('<')) {
                          // Collect all consecutive HTML lines
                          let html = '';
                          const startIdx = i;
                          while (i < question.solutionSteps!.length && question.solutionSteps![i].trimStart().startsWith('<')) {
                            html += question.solutionSteps![i];
                            i++;
                          }
                          groups.push({ isHtml: true, content: html, startIdx });
                        } else {
                          groups.push({ isHtml: false, content: step, startIdx: i });
                          i++;
                        }
                      }
                      return groups.map((group, gi) =>
                        group.isHtml ? (
                          <div
                            key={gi}
                            className="solution-html-block rounded-md overflow-x-auto py-1"
                            dangerouslySetInnerHTML={{ __html: group.content }}
                          />
                        ) : (
                          <div key={gi} className="flex items-start gap-2 bg-white/50 dark:bg-black/10 px-2.5 py-1 rounded-md font-mono">
                            <span className="shrink-0 text-gray-400 font-bold">{group.startIdx + 1}.</span>
